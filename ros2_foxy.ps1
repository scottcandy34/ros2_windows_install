...
function Add_Links {
    ...
    $Link = ([Environment]::GetFolderPath("Desktop") + "\ROS2 Foxy Terminal.lnk")
...
function Standard-Install {
    ...
    # Install ROS2
    $release = Get-Release -Search "foxy"
    if ($release -eq $null) {
    ...
    Add_Links -Path "$ROS_DIR\ros2_foxy"
}
...
function Alternate-Install {
    ...
    Add_Links -Path "C:\opt\ros\foxy\x64\"
}
...
function Uninstall-Ros {
    ...
    if (Test-Path -Path "C:\opt") {
        $ROS_START = "C:\opt\ros\foxy\x64\local_setup.ps1"
        $env:ChocolateyInstall = "c:\opt\chocolatey"
        choco uninstall -y ros-foxy-desktop
        $ROS_INSTALL_COUNT = (Get-ChildItem -Directory -Path $ROS_DIR | Measure-Object).Count
        if ($ROS_INSTALL_COUNT -eq 0) {
...
function Uninstall-Dep {
    ...
    python -m pip uninstall -Y pydot PyQt5
    python -m pip uninstall -y catkin_pkg cryptography empy ifcfg lark-parser lxml netifaces numpy opencv-python pyparsing==2.4.7 pyyaml setuptools==59.6.0 rosdistro
...
$Standard = "C:\dev\ros2_foxy"
$Alternate = "C:\opt\ros\foxy"
if ((Test-Path -Path $Standard) -or (Test-Path -Path $Alternate)) {
    $title    = 'You already have ROS2 Foxy installed.'
