...
function Add_Links {
    ...
    "set RMW_IMPLEMENTATION=rmw_fastrtps_cpp`n" + (Get-Content $Startup -Raw) | Set-Content $Startup

    # Creating Desktop Shortcut
    $Link = ([Environment]::GetFolderPath("Desktop") + "\ROS2 Iron Terminal.lnk")
...
function Standard-Install {
    ...
    # Install ROS2
    $release = Get-Release -Search "iron"
    if ($release -eq $null) {
...
function Alternate-Install {
    Write-Output "Check at this site to see if a new build for iron exists yet."
    Write-Output "https://dev.azure.com/ros-win/ros-win/_build?pipelineNameFilter=iron"
    pause
    exit
    ...
    choco upgrade ros-iron-desktop -y --execution-timeout=0 -pre

    Add_Links -Path "C:\opt\ros\iron\x64\"
}
...
function Uninstall-Ros {
    ...
    if (Test-Path -Path $ROS_DIR) {
        $ROS_DIR_INSTALL = "$ROS_DIR\ros2_iron"
        $ROS_START = "$ROS_DIR_INSTALL\local_setup.ps1"
        if (Test-Path -Path $ROS_DIR_INSTALL) {
            Uninstall -Path $ROS_DIR_INSTALL -Title "ROS2 Iron Standard"
        }
    ...
    if (Test-Path -Path "C:\opt") {
        $ROS_START = "C:\opt\ros\iron\x64\local_setup.ps1"
        $env:ChocolateyInstall = "c:\opt\chocolatey"
        choco uninstall -y ros-iron-desktop
...
function Uninstall-Dep {
    ...
    Uninstall -Path "C:\xmllint" -Title "XML Lib"
    ...
    Remove-Path -RemovePath "C:\xmllint"
}
...
$Standard = "C:\dev\ros2_iron"
$Alternate = "C:\opt\ros\iron"
if ((Test-Path -Path $Standard) -or (Test-Path -Path $Alternate)) {
    $title    = 'You already have ROS2 Iron installed.'
